...
const TEAM_EMAILS = ['loans@tribunefunding.com', 'tyler@tribunefunding.com'];
...
function generateEmailBody(data, type) {
  ...
  const propertyTypeLabels = { 'single_family': 'Single Family', 'multi_family': 'Multi-Family', 'condo': 'Condo', 'townhouse': 'Townhouse', 'mixed_use': 'Mixed-Use', 'commercial': 'Commercial', 'land': 'Land', 'other': 'Other' };
  const loanPurposeLabels = { 'purchase': 'Purchase', 'refinance': 'Refinance', 'cash_out': 'Cash-Out', 'fix_and_flip': 'Fix & Flip', 'construction': 'Construction', 'bridge': 'Bridge', 'land': 'Land' };
  let subject = type === 'submission' ? `New Loan Application - ${applicationId} - ${borrowerProfile.borrowerName}` : `Loan Pre-Qualification Match - ${applicationId}`;
  const body = `
═══════════════════════════════════════════════════════════════
...
app.post('/api/submit', async (req, res) => {
  ...
  if (!borrowerProfile || !applicationId) return res.status(400).json({ error: 'Missing required fields' });
  try {
    const { data, error } = await supabase.from('loan_submissions').insert({
      application_id: applicationId, borrower_name: borrowerProfile.borrowerName,
      borrower_email: borrowerProfile.borrowerEmail, borrower_phone: borrowerProfile.borrowerPhone || null,
      property_type: borrowerProfile.propertyType, property_state: borrowerProfile.propertyState,
      property_value: borrowerProfile.propertyValue || null, loan_amount: borrowerProfile.loanAmount,
      loan_purpose: borrowerProfile.loanPurpose || null, credit_score: borrowerProfile.creditScore || null, status: 'pending'
    }).select().single();
    if (error) return res.status(500).json({ error: 'Failed to store submission in database' });
    console.log(`[${new Date().toISOString()}] Submission ${applicationId} stored in database`);
    transporter.sendMail({
      from: `"${SENDER_NAME}" <${SENDER_EMAIL}>`, to: TEAM_EMAILS.join(', '),
    ...
    res.json({ success: true, message: 'Application submitted successfully', applicationId, submissionId: data?.id });
  } catch (error) {
...
app.post('/api/send-to-lender', async (req, res) => {
  try {
    ...
    if (!borrowerProfile || !lender || !applicationId) return res.status(400).json({ error: 'Missing required fields' });
    const { data: submission } = await supabase.from('loan_submissions').select('id').eq('application_id', applicationId).single();
    if (submission) {
      supabase.from('lender_contacts').insert({
        submission_id: submission.id, lender_id: lender.id || null,
        lender_company: lender.company, lender_email: lender.email, status: 'sent'
      }).then(() => console.log(`[${new Date().toISOString()}] Lender contact recorded`)).catch(err => console.error('Contact error:', err.message));
    }
    const propertyTypeLabels = { 'single_family': 'Single Family', 'multi_family': 'Multi-Family', 'condo': 'Condo', 'townhouse': 'Townhouse', 'mixed_use': 'Mixed-Use', 'commercial': 'Commercial', 'land': 'Land', 'other': 'Other' };
    const loanPurposeLabels = { 'purchase': 'Purchase', 'refinance': 'Refinance', 'cash_out': 'Cash-Out', 'fix_and_flip': 'Fix & Flip', 'construction': 'Construction', 'bridge': 'Bridge', 'land': 'Land' };
...
═══════════════════════════════════════════════════════════════
LOAN OPPORTUNITY SUMMARY - Application ID: ${applicationId}
═══════════════════════════════════════════════════════════════
Property Type: ${propertyTypeLabels[borrowerProfile.propertyType] || borrowerProfile.propertyType}
Location: ${borrowerProfile.propertyState} | Property Value: ${formatCurrency(borrowerProfile.propertyValue)}
Loan Amount: ${formatCurrency(borrowerProfile.loanAmount)} | Purpose: ${loanPurposeLabels[borrowerProfile.loanPurpose] || borrowerProfile.loanPurpose}
LTV: ${borrowerProfile.propertyValue > 0 ? Math.round((borrowerProfile.loanAmount / borrowerProfile.propertyValue) * 100) + '%' : 'N/A'}
Credit Score: ${borrowerProfile.creditScore || 'Not provided'}
Monthly Income: ${borrowerProfile.monthlyRent ? formatCurrency(borrowerProfile.monthlyRent) : 'Not provided'}
To express interest, contact: loans@tribunefunding.com | CC: tyler@tribunefunding.com
Reference Application ID: ${applicationId}
Tribune Funding Network
    `.trim();
    transporter.sendMail({
      from: `"${SENDER_NAME}" <${SENDER_EMAIL}>`, to: lender.email,
      cc: TEAM_EMAILS.join(', '), subject: lenderSubject, text: lenderBody
    }).catch(emailError => console.error('Email error:', emailError.message));
    res.json({ success: true, message: 'Email queued to lender and team' });
  } catch (error) {
