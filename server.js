...
// Route: Send to individual lender - Email sent without borrower contact info
  ...
  try {
    ...
    if (submission) {
      supabase
        ...
        .then(() => console.log(`[${new Date().toISOString()}] Lender contact recorded for ${applicationId}`))
        .catch(err => console.error('Contact record error:', err.message));
    }

    // Email to lender - WITHOUT borrower contact info
    const propertyTypeLabels = {
      'single_family': 'Single Family', 'multi_family': 'Multi-Family',
      'condo': 'Condo', 'townhouse': 'Townhouse', 'mixed_use': 'Mixed-Use',
      'commercial': 'Commercial', 'land': 'Land', 'other': 'Other'
...
Tribune Funding Network
    ...
    // Send email fire-and-forget to lender with CC to team
    transporter.sendMail({
      from: `"${SENDER_NAME}" <${SENDER_EMAIL}>`,
      to: lender.email,
      cc: 'lendertribune@gmail.com, loans@tribunefunding.com',
      subject: lenderSubject,
      text: lenderBody
    }).catch(emailError => console.error('Email error:', emailError.message));

    // Return success immediately
    res.json({ success: true, message: 'Email sent to lender' });
  } catch (error) {
